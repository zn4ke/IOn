...
module.exports = function (socket, io) {
    ...
    socket.on('event:push', function(data){
        io.sockets.to( data.id ).emit('event:push', data)
    });
    socket.on('event:answer', function(data){
        console.log('answer from user:', socket.handshake.user._id);
        console.log('data:', data);
        data.user = socket.handshake.user._id;
        io.sockets.to( data.id ).emit('event:answer', data);
        console.log('sockets in event')
        console.log(io.sockets.clients(data.id))
    });

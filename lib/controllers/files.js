...
var fileApi = {
    ...
    listImages: function(req, res){ // for ck editor image browser

        File.find({ own: req.user._id, type: {$regex : ".*image.*"}}, function( err, docs){
            var imageList = [];
            _.each(docs, function(doc){
                var image = {
                    image: doc.src,
                    thumb: doc.thumbSrc,
                    folder: "testfolder" + Math.round(Math.random())
                };
                imageList.push(image)
            });
            if (!err) res.json( imageList )
            else res.json( err )
        });
    },
